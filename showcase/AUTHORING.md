# Portable graph authoring review

One pattern_deep_work owns the coherent export repair and preview. Three required criteria evaluate behavior, simplicity, and operator clarity, with a 0.85 threshold and up to three cycles. No feature or rubric was added during packaging.

Packaging changes: the repository path is relative; the command locates the committed acceptance tools; the static application guide is APP_GUIDE.md; a prohibition protects the graph and showcase inputs now stored alongside the app. These tools use a separate fixture and oracle but share the repository filesystem. The original run used acceptance tools outside the product workspace.

The original runtime-prose table was reviewed before the original graph. The complete portable table below is written before this graph JSON. Outcome prose does not describe graph topology or authoring tasks.
