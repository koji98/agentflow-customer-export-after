...
The original application source, UI, fixture, and product tests come unchanged from the broken dashboard at source commit `e875f94c455716bcd33162b5f95a4224897d33ec`. Root documentation, package scripts, Node metadata, and setup tooling were added for onboarding. The original app README is preserved as APP_GUIDE.md. EXPORT_PREVIEW.md is the exact additional task contract present before the expanded run; it is committed here so future runs start clean.
...
Main-branch setup improvements add `.nvmrc` (24.18.0), strict Node 24 metadata, a lockfile, a setup doctor, and CI. They do not alter the product source or recorded graph/rubric evidence. Use the root README for current setup; APP_GUIDE.md remains the original task context. The original snapshot tags have not moved.
